...
const GetPrime = () => {
  return (
    <div className="bg-gray-100 min-h-screen flex justify-center items-center">
      <div className="max-w-screen-xl w-full p-4 sm:p-6 lg:p-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        {subscriptionPlans.map((plan, index) => (
          <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col justify-between">
            <div className="p-6">
              <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4">{plan.title}</h2>
              <p className="text-sm text-gray-600 mb-4">{plan.description}</p>
              <div className="flex items-center justify-center mb-4">
                <span className="text-2xl md:text-3xl font-semibold text-gray-800 mr-1">{plan.price}</span>
                ...
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-500 mr-2" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M5.293 11.293a1 1 0 0 1 1.414 0L10 14.586l3.293-3.293a1 1 0 1 1 1.414 1.414l-4 4a1 1 0 0 1-1.414 0l-4-4a1 1 0 0 1 0-1.414z" clipRule="evenodd" />
                    </svg>
                    {feature}
                  </li>
                ))}
            ...
            <button className={`text-white text-sm py-2 rounded-b-md hover:bg-green-900 transition duration-300 ${plan.buttonBgColor}`}>
              {plan.buttonText}
            </button>
          </div>
        ))}
